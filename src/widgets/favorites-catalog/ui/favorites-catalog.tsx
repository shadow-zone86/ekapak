    ...
    <div className="favorites-catalog flex flex-col gap-4">
      {isError && errors.length > 0 && (
        <div className="favorites-catalog__error mb-4 rounded-lg bg-red-50 p-4 text-red-800">
          ...
          <ul className="favorites-catalog__error-list list-disc list-inside">
            {errors.map((error, index) => (
              <li key={index} className="favorites-catalog__error-item">{error?.message || String(error)}</li>
            ))}
          </ul>
        </div>
      )}
      {isLoading ? (
        <div className="favorites-catalog__skeleton grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="favorites-catalog__skeleton-item h-80 animate-pulse rounded-lg bg-gray-200"
            />
          ))}
        </div>
      ) : products.length > 0 ? (
        <div className="favorites-catalog__grid grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
        ...
        <div className="favorites-catalog__empty flex flex-col items-center justify-center py-16 text-center">
          <div className="favorites-catalog__empty-icon mb-4">
            <svg
              className="favorites-catalog__empty-icon-svg mx-auto h-16 w-16 text-gray-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <H1 className="favorites-catalog__empty-title mb-2 text-xl font-semibold text-gray-900">В избранном пока пусто</H1>
          <PText className="favorites-catalog__empty-text text-gray-500">
